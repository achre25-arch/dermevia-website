...
const CONFIG = {
  ...
  RATE_LIMIT_ENDPOINT: '/.netlify/functions/validate-rate-limit', // إن لم تكن موجودة، لا بأس
  PHONE_COOLDOWN_MS: 24 * 60 * 60 * 1000, // 24h ليتطابق مع السيرفر
...
const i18n = {
  ...
  fr: { 
    ...
    testimonialsSubtitle: "Avis vérifiés...",
...
// =========== ORDER SENDER (يتعامل مع 429 ويُرجِع ttlMs) ===========
  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (response.status === 429) {
        ...
        return { success: false, error: 'rate_limit', message: json?.message || i18n[getLang()].rateLimitText, ttlMs, data: json };
      }
      if (response.status === 400) {
        return { success: false, error: 'validation', message: json?.error || 'Invalid data' };
      }
...
function showServerRateLimitCountdown(ttlMs, serverMessage) {
  const lang = getLang();
  const baseText = i18n[lang].rateLimitText; // "يرجى المحاولة بعد:"
  const waitMs = Math.max(0, Number(ttlMs || CONFIG.PHONE_COOLDOWN_MS));
  const end = Date.now() + waitMs;

  // أنشئ صندوقاً تلقائياً إن لم تكن لديك عناصر alreadyBox/alreadyText
  ...
  // أوقف أي عدّ سابق وابدأ عدّاً جديداً
  ...
  const update = () => {
    ...
    if (left <= 0) {
      ...
      // إخفِ الصندوق تلقائياً عند انتهاء العدّ
...
async function handleOrderSubmissionSecure(e) {
  ...
  try {
    ...
    if (result.success) {
      ...
      // تحويل إلى صفحة الشكر/ملخص الطلب
    ...
    if (result.error === 'rate_limit') {
      // هنا سنعرض العدّ التنازلي بدلاً من الرسالة العامة
  ...
  try {
    ...
    if (result.success) {
      // حفظ محلي + تتبّع + تحويل للصفحة order.html
      saveOrderAdvanced(phone);
      saveOrderLocally(order);
      try {
        if (typeof fbq === 'function') {
